...
//kafe code starts
        sendAdvEvt = new cMessage("adv evt", SEND_ADVERTISEMENT_EVT);
...
{
    wsm->setRecipientAddress(rcvId);        //define in BaseFrame1609_4.msg
    wsm->setBitLength(headerLength);        //

    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        bsm->setSenderPos(curPosition);     //define in DemoSafety Message.msg
        bsm->setSenderSpeed(curSpeed);      //define in DemoSafety Message.msg
        bsm->setPsid(-1);                   //define in BaseFrame1609_4.msg
        bsm->setChannelNumber(static_cast<int>(Channel::cch));      //define in BaseFrame1609_4.msg
        bsm->addBitLength(beaconLengthBits);
        wsm->setUserPriority(beaconUserPriority);                   //define in BaseFrame1609_4.msg
    }
    else if (DemoServiceAdvertisment* wsa = dynamic_cast<DemoServiceAdvertisment*>(wsm)) {
        wsa->setChannelNumber(static_cast<int>(Channel::cch));      //define in BaseFrame1609_4.msg
        wsa->setTargetChannel(static_cast<int>(currentServiceChannel));   //define in DemoServiceAdvertisement.msg
        wsa->setPsid(currentOfferedServiceId);                           //define in BaseFrame1609_4.msg
        wsa->setServiceDescription(currentServiceDescription.c_str());  //define in DemoServiceAdvertisement.msg
    }
    else {
        if (dataOnSch)
